    ...
    public partial class FormOperationExit : Form
    {
        ...
        private void BtnCalculateFee_Click(object sender, EventArgs e)
        {
            if (LstParking.SelectedIndex >= 0)
            {
                ...
                {
                    String amount = $"(One hour = ${(_parkingCalculated.Large ? l_hour : s_hour)}) + (Additional hour = {Math.Floor(ts.TotalHours) - (ts.Minutes == 0 ? 1 : 0)} x ${(_parkingCalculated.Large ? l_add : s_add)}) = ${total.ToString("N2")}";
